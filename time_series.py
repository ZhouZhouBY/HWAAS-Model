...
def mean_absolute_percentage_error(y_true, y_pred):
    # print("res:", np.mean(np.abs((y_true - y_pred) / y_true)) * 100)
...
class HoltWinters:
    ...
    def _plot_predicted_deviation(self):

        plt.figure(figsize=(20, 10))
        plt.plot(self.predicted_deviation)
        plt.grid(True)
        plt.axis('tight')
        plt.title("Predicted Deviation")
        # plt.show()

    def triple_exponential_smoothing(self, plot_results=False, plot_predicted_deviation=False, *args, **kwargs):
        """
        ...
        if plot_predicted_deviation:
            self._plot_predicted_deviation()

        if plot_results or plot_predicted_deviation:
            plt.show()

        return self.result

    def get_best_parameters(self, inplace=True, loss_function=mean_absolute_percentage_error, n_folds=3):
        """
...
if __name__ == '__main__':
    currency = pd.read_csv('currency1.csv', index_col=['Time'], parse_dates=['Time'])

    data = currency.GEMS_GEMS_SPENT[:-453]
